...
which turn on MicroProfile Fault Tolerance in Open Liberty.
...
The following two steps are setting up the dynamic configuration on the `system` service and its client.
...
An IOException is thrown to simulate the `system` service being unavailable.
...
The `@Fallback` annotation dictates a method to call when encounters a failed execution. In this
...
browser to the the `system` service URL, which is `http://localhost:9080/system/properties`,
...
`resources/CustomConfigSource.json` file. This turns the `system` service down in maintenance.
