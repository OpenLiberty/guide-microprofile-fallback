...
ifdef::cloud-hosted[]
To see the `inventory` service with a localhost hostname run the following curl command.
```
...
The [hotspot=fallbackForGet file=1]`fallbackForGet()` method, which is the designated fallback method, is called when the **system** service is not available.
endif::[]
