...
:page-seo-title: Building fault-tolerance microservices with the MicroProfile Fault Tolerance @Fallback annotation
:page-seo-description: A tutorial and an example on how to build resilient microservices using MicroProfile Fault Tolerance and obtain metrics for the Fault Tolerance methods with MicroProfile Metrics.
...
The MicroProfile Fault Tolerance API is included in the MicroProfile dependency specified by your [hotspot file=4]`pom.xml` file. 
...
the [hotspot=21-22 file=0]`isInMaintenance()` condition, which determines that the system properties are returned only if you
...
Note that the [hotspot=mpMetrics]`mpMetrics` feature requires SSL and the configuration has been provided for you. The [hotspot=quickStartSecurity]`quickStartSecurity` and [hotspot=keyStore]`keyStore` configuration elements provide basic security to secure the
server. When you visit the `/metrics` endpoint, use the credentials defined in the server configuration to log in to view the data for the fault tolerance methods.

You can learn more about MicroProfile Metrics in the https://openliberty.io/guides/microprofile-metrics.html[Providing metrics from a microservice^] guide. You can also learn more about the MicroProfile Fault Tolerance and Metrics integration in the https://github.com/eclipse/microprofile-fault-tolerance/releases[MicroProfile Fault Tolerance specification^].
...
To see the application metrics, point your browser to https://localhost:9443/metrics/application[https://localhost:9443/metrics/application^]. Log in as the `admin` user with `adminpwd` as the password.
...
data indicates that the `@Fallback` annotated method [hotspot=19-21 file=1]`get()` has a total of 2 calls
including the previous call before turning the `system` service in maintenance.
The `ft_io_openliberty_guides_inventory_inventory_manager_get_fallback_calls_total` data
indicates that the [hotspot=23-31 file=1]`fallbackForGet()` method has 1 call.
...
The [hotspot=21]`@BeforeEach` and [hotspot=27]`@AfterEach` annotations indicate that this method executes before or after the other test case.
...
Tests run: 1, Failures: 0, Errors: 0, Skipped: 0, Time elapsed: 0.396 sec - in it.io.openliberty.guides.inventory.InventoryEndpointIT
...
in the [hotspot]`FaultToleranceIT.java` file. Rerun the tests to see a test failure occur for the [hotspot=34-54]`testFallbackForGet()` test case.
