...
get the system properties for an incorrect hostname (`localhot`). Then, it confirms that the fallback method has not been
called by asserting that the response's status code is `404` with an error message in the response body.

The [hotspot=Test]`@Test` annotations indicate that the methods automatically execute when your test class runs.
