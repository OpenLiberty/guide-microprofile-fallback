...
SystemResource.java
[source, java, linenums, role='code_column tags=503_response']
----
...
SystemClient.java
[source, java, linenums, role='code_column tags=client']
----
...
[source, java, linenums, role='code_column tags=add_fallback']
----
...
The [hotspot=Before]`@BeforeEach` and [hotspot=After]`@AfterEach` annotations indicate that this method runs either before or after the other test case.
...
The [hotspot=Test]`@Test` annotation indicates that the method automatically executes when your test class runs.
...
[source, java, linenums, role='code_column hide_tags=javadoc tags=ft_testing']
----
...
in the [hotspot]`FaultToleranceIT.java` file. Rerun the tests to see that a test failure occurs for the [hotspot=testFallbackForGet]`testFallbackForGet()` test case.
