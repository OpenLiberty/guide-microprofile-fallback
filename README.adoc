...
when you add the MicroProfile Metrics feature added to the server.
...
Note that the `@Fallback` annotated method [hotspot=19-21 file=1]`get()` has a total 2 calls,
and the [hotspot=23-31 file=1]`fallbackForGet()` method has 1 call.
