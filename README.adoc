...
:page-description: Learn how to add retry and fallback behavior to microservice dependencies to manage the impact of failures using MicroProfile Fault Tolerance.
...
The public `getProperties()` method, which is in the `InventoryManager.java` class,
calls the `getPropertiesHelper()` method. You will look into this class in more detail in the next section.
...
Lastly, this test case asserts whether the system properties that returned from the first request were more than the second.
