...
The [hotspot=Fallback]`@Fallback` annotation provides two parameters, [hotspot=33]`applyOn` and [hotspot=34]`skipOn`, which allow you to configure
...
To trigger [hotspot=fallbackForGet]`fallbackForGet()` method when the system service is unavailable, the [hotspot=33]`applyOn` parameter is given the value `IOException.class`.
To rethrow the `UnknownHostException` instead of triggering the [hotspot=fallbackForGet]`fallbackForGet()` method, the [hotspot=34]`skipOn` parameter is given the value `UnknownHostException.class`.
