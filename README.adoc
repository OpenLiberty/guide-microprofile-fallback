...
You will add fault tolerance to the `inventory` service so that it reacts accordingly with the `system`
service being unavailable.

You will use the `@Fallback` annotations from the MicroProfile (MP) Fault Tolerance
...
Because the system service is now in maintenance, you see the cached properties from the systems for this localhost.
For simplicity, only the OS name and user name are stored in the cache for each host.
...
the `inMaintenance()` condition, which determines the system properties are returned only if you
