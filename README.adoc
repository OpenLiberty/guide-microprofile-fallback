...
You will update an `inventory` service (a service which stores a list of all previously registered hosts and 
information about each hosts system properties, stored from the `system` service) to be able to recognise when 
the `system` service is in maintenance and use MicroProfile Fault Tolerance methods to address this.
You will also update the `system` service in order to be able to simulate the service being brought down for 
maintenance 

You will use the `@Retry` and `@Fallback` Fault Tolerance methods to define a criteria on when to retry and when to 
...
Now navigate to the `CustomConfigProperties.json` file in the `finish/resource` directory and change the property
...
Once you are done checking out the application, go to the `CustomConfigProperties.json` file again and change the
...
Begin by taking a look at the `pom.xml` file to check the required dependency. The `microprofile-fault-tolerance-api`
...
Create the inventory manager class in the `start/src/main/io/openliberty/guides/inventory/InventoryManager.java` file:
...
the `io_openliberty_guides_system_inMaintenance` property value to `true` in the `start/resource/CustomConfigProperties.json` file.
