...
Now navigate to the `CustomConfigSource.json` file in the `finish/resource` directory and change the property
...
even though
`system` service is still in maintenanace from previous test case. Then, it reads the retry counter in
