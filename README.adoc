...
To enable fault tolerance in the application, create the `SystemResource` class in the 
...
The `@BeforeClass` annotation is placed on a method that executes before any of the other test cases. In
...
The `@Before` and `@After` annotations are placed on methods that execute before and after any 
other test case. These methods are generally used to perform any setup and teardown tasks. In this case,
the setup method creates a JAX-RS client, which makes HTTP requests to the `inventory` service. This
client must also be registered with a `JsrJsonpProvider` JSON-P provider to process JSON resources. 
...
host name when the `system` service is available. Then, it puts the `system` service on maintenance. The
`inventory` service returns the properties after making a connection to the `system` service.
Afterward, it makes a second request to the `inventory` service to return the system properties for that
host name. The fallback method from the `inventory` service is called to return the cached properties. 
Lastly, this test case asserts whether the system properties that are returned from the first request were more than the second
request.

* The `testRetryGettingSystemProperties()` test case also sends a request to  the `inventory` service to try to retrieve the
system properties. Then, it reads the retry counter in the `/inventory/retries` endpoint to assert the endpoint 
with the number of retries that happened to get the system properties from the `inventory` service. To get the system properties, the 
`inventory` service retries the same number of times as stated by the `maxRetries` attribute
in the `@Retry` annotation. However, the service first calls on the 
annotated method and assumes that the `system` service is available. As a result, this method has four total calls.
...
You built and tested a micorservice application with MicroProfile Fault Tolerance and Open Liberty.
