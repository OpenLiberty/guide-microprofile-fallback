...
`ft_io_openliberty_guides_inventory_inventory_manager_get_invocations_failed_total` metric has incremented
...
`ft_io_openliberty_guides_inventory_inventory_manager_get_fallback_calls_total` metrics will
increment. Similarily, if the `io_openliberty_guides_inventory_fallbackBroken`
property is set to `true`, the `ft_io_openliberty_guides_inventory_inventory_manager_get_invocations_failed_total`
metric will increment when the `http://localhost:9080/inventory/systems/localhost` is visited.
