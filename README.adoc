...
Add this dependency to use MicroProfile Fault Tolerance API in your code to use the
fault tolerance policies in your microservices.

Enable the `mpFaultTolerance-1.0` feature in your `src/main/liberty/config/server.xml` server configuration
to turn on MicroProfile Fault Tolerance in Open Liberty, which has been done for you.
...
A condition is added in this method to stage the fault tolerance in the application.
...
Thus, the method prints out a warning
message in the browser. Otherwise, this method returns the cached property values.

You have now successfully set up your fault tolerant microservices.
...
browser to the `http://localhost:9080/system/properties` URL for the `system` service,
which retrieves the system properties for the specific `localhost`.
Notice that the results from the two URLs are identical because `inventory` service gets its results from
...
`resources/CustomConfigSource.json` file. After saving the file, go back to your browser and
refresh to the `http://localhost:9080/inventory/systems/localhost` URL to view the cached version of
the properties. The `fallbackForGet()` method, which is the fallback designated method is being called
...
You see the service displays a 503 HTTP response code.
