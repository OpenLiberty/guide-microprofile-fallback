...
A method called `getPropertiesHelper()` is included in this class. This method makes a request to the
`system` service and checks for the response. If a `Status.OK` response that corresponds to the
...
The public `getProperties()` method, which is used 
in the `InventoryManager.java` class, calls the `getPropertiesHelper()` method. You will look into this class in more detail in the next section.
...
Because of the 503 HTTP response that was returned,
the `inventory` microservice is now able to recognise that the `system` microservice was taken
down for maintenance. As a result, the response throws an IOException.
...
The `@Retry` annotation dictates the conditions that determine if the microservice should reattempt to establish a connection
to the `system` service and how many times it should retry. In this example, the microservice retries three times.
The `@Fallback` annotation dictates which method to call when the retries are unsuccessful. In this
example, you can use the `fallbackForGet` method.

The `fallbackForGet()` method, which is added as the designated fallback method for the original
`get()` method, checks to see if the system's properties exist in the inventory. If the properties do 
not exist, the application has likely been unsuccessful in reachig the `system`
service to get the system’s properties. Thus, the method prints out a JSON warning 
...
`resource/CustomConfigSource.json` file. After saving the file, go back to your browser and 
