...
ifdef::cloud-hosted[]
To access the **inventory** service with a localhost hostname, run the following curl command from the terminal in the IDE:
```
...
// Following context for the cloud-hosted guide:
ifdef::cloud-hosted[]
When the server is running, run the following curl command from the terminal in the IDE:
```
curl http://localhost:9080/inventory/systems/localhost
```
{: codeblock}
...
To see the application metrics, run the following curl commmand from the terminal in the IDE. This command will Log in using **admin** user, and you will have to enter **adminpwd** as the password.
```
...
// Following context for the cloud-hosted guide:
ifdef::cloud-hosted[]
After saving the file, run the following curl command from the terminal in the IDE to view the cached version of the properties:
```
curl http://localhost:9080/inventory/systems/localhost
```
{: codeblock}
...
// Following context for the cloud-hosted guide:
ifdef::cloud-hosted[]
To see that the **system** service is down, run the following curl command from the terminal in the IDE:
```
...
// Following context for the cloud-hosted guide:
ifdef::cloud-hosted[]
Run the following curl command again from the terminal in the IDE and enter **adminpwd** as the password:
```
curl -k -u admin https://localhost:9443/metrics/application
```
{: codeblock}
