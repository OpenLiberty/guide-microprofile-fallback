...
The service displays the system properties for a specifichost.
The `inMaintenance()` condition determines returning the system properties only if you
...
Otherwise, the service returns an `Status.SERVICE_UNAVAILABLE` message, which makes it unavailable.
...
specific host. Notice the results from the two URLs are identical.
