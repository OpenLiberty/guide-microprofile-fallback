...
Next, the `src/main/java/io/openliberty/guides/inventory/client/ExceptionMapper.java` class includes the `getPropertiesHelper()` method to make a request to the	+includes the `handles()` and  `toThrowable()` methods to check for the response in order to handle the exceptions.
