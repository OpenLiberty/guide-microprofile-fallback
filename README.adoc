...
----
#Edit the `CustomConfigSource` configuration file.#
...
The MicroProfile Fault Tolerance API was added as a dependency to your [hotspot=1-253 file=4]`pom.xml` file. Look for
the dependency with the [hotspot=113-118 file=4]`mpFaultTolerance` artifact ID. Adding this dependency allows you to use the 
fault tolerance policies in your microservices.

You can also find the [hotspot=9 file=3]`mpFaultTolerance` feature in your [hotspot=1-15 file=3]`src/main/liberty/config/server.xml` server configuration,
...
First, the [hotspot=1-27 file=0]`src/main/java/io/openliberty/guides/system/SystemResource.java` file has
...
to `false` in the [hotspot=1-2 file=1]`CustomConfigSource` file.
Otherwise, the service returns a [hotspot=23-25 file=0]`Status.SERVICE_UNAVAILABLE` message, which makes it unavailable.

Next, the [hotspot=1-25 file=2]`src/main/java/io/openliberty/guides/inventory/client/SystemClient.java` file
...
[role="code_command hotspot=13-56", subs="quotes"]
----
...
[role="code_command hotspot=2-2", subs="quotes"]
----
...
the [hotspot=2]`io_openliberty_guides_system_inMaintenance` property value to `true` in the
...
the properties. The [hotspot=23-31]`fallbackForGet()` method, which is the designated fallback method, is called
...
[role="code_command hotspot=16-59", subs="quotes"]
----
