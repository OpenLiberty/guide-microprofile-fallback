...
where the `isInMaintenance()` condition gets the configuration property from the `io_openliberty_guides_system_inMaintenance` key.
...
Next, you enable the [hotspot file=2]`SystemClient` class to throw an [hotspot=22 file=2]`IOException`
...
Finally, the `get()` method in the [hotspot file=5]`InventoryManager` class calls the [hotspot=21-23 file=2]`getProperties()` method
