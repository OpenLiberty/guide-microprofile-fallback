...
You will use the `@Retry` and `@Fallback` Fault Tolerance annotations to define a criteria on when to retry and when to
...
The implementation of the application and its services are provided for you in the `src` directory.
...
For ease of use, the two microservices being run in this application are being "served up"
by the same server.
...
up and down dynamically using dynamic configuration with MP config. If you want
...
To do this, create the `system` client class in the
...
down for maintenance through the 503 HTTP response retruned. As a result, it has thrown an IOException.
...
The `@Retry` annotation dictates on on what conditions we should reattempt to establish a connection
...
Next create the `inventory` resource class in `src/main/java/io/openliberty/guides/inventory/InventoryResource.java` file:
