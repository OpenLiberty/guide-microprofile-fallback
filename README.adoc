...
a failed execution. You will also learn how to view metrics for any method annotated with
...
Next, point your browser to the `https://localhost/metrics/application` MicroProfile Metrics endpoint.
Log in as the `admin` user with `adminpwd` as the password. You can see the application metrics in a
text format.

See the following sample outputs for the Fallback metrics:
...
Now, open the `resources/CustomConfigSource.json` file and change the `io_openliberty_guides_inventory_fallbackBroken`
from `false` to `true` and save the file. This simulates the fallback method being broken. When returning
back to the `http://localhost:9080/inventory/systems/localhost` URL, notice how an error is displayed.
Next, return back to the `https://localhost/metrics/application` URL. Notice how the
`ft_io_openliberty_guides_inventory_inventory_manager_get_invocations_failed_total` has incremented
...
condition back to its original value. In addition, change the `io_openliberty_guides_inventory_fallbackBroken`
...
// =================================================================================================
// Using MP Metrics with MP Fault Tolerance
// =================================================================================================

== Using MP Metrics with MP Fault Tolerance

When integrating MP Fault Tolerance with MP Metrics, all Fault Tolerance capabilities are monitored.
This means that metrics are automatically added for any method annotated with the `@Fallback` annotation.
For the purpose of this guide, MP Metrics has already been configured for you. To learn more about
MP Metrics, check out the https://openliberty.io/guides/microprofile-metrics.html[Providing metrics from a microservice]
guide. 

Open the `src/main/liberty/config/server.xml` file. The `quickStartSecurity` and `keyStore` configuration
elements provide basic security to secure the server. When you visit the `/metrics/application` endpoint,
use the credentials defined in the server configuration to log in to view the data.

MicroProfile Metrics provides three metrics when used in conjunction with fallback:

* The `ft_io_openliberty_guides_inventory_inventory_manager_get_invocations_total` metric counts the 
number of times the method annotated with the `@Fallback` annotation is called.

* The `ft_io_openliberty_guides_inventory_inventory_manager_get_invocations_failed_total` metric counts
the number of times the method annotated with the `@Fallback` annotation is called and threw a `Throwable`, 
even after the fallback action was processed.

* The `ft_io_openliberty_guides_inventory_inventory_manager_get_fallback_calls_total` metric counts
the number of times the fallback handler or method is called.

Open the `src/main/java/io/openliberty/guides/inventory/InventoryManager.java` file. Notice that in
the `fallbackForGet` method, if the `isFallbackBroken()` method returns `true`, a `RuntimeException`
will be thrown. This if statement is strictly to simulate the fallback method being broken so that
the `ft_io_openliberty_guides_inventory_inventory_manager_get_invocations_failed_total` metric can
be demonstrated. 

If you would like to disable MP Fault Tolerance metrics capabilities, you can do so through the 
`MP_Fault_Tolerance_Metrics_Enabled` property in the `resources/CustomConfigSource.json` file. If you 
change the value of the `MP_Fault_Tolerance_Metrics_Enabled` property, be sure to save the file and 
...
You can see the fallback metrics by navigating to the `https://localhost/metrics/application` URL and 
logging in as `admin` with the password `adminpwd`. If the `io_openliberty_guides_system_inMaintenance`
property is set to `true` in the `resources/CustomConfigSource.json`, whenever the `http://localhost:9080/inventory/systems/localhost`
URL is visited, the `ft_io_openliberty_guides_inventory_inventory_manager_get_invocations_total` and 
`ft_io_openliberty_guides_inventory_inventory_manager_get_fallback_calls_total` properties should
increment when viewing the metrics. Similarily, if the `io_openliberty_guides_inventory_fallbackBroken`
property is set to `true`, the `ft_io_openliberty_guides_inventory_inventory_manager_get_invocations_failed_total`
should increment when the `http://localhost:9080/inventory/systems/localhost` is visited.

When you're done, change the value of the `io_openliberty_guides_system_inMaintenance` and 
`io_openliberty_guides_inventory_fallbackBroken` properties back to `false` in the `resources/CustomConfigSource.json` 
file.
...
You just learned how to build a fallback mechanism for a microservice with MicroProfile Fault Tolerance 
and wrote a test to validate it. You also learned how to view the fallback metrics when using
MicroProfile Fault Tolerance in conjunction with MicroProfile Metrics. 
