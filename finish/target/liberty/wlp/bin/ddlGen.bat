...
if exist "%JAVA_HOME%\lib\modules" set JVM_ARGS=--add-modules java.se.ee --add-opens java.base/java.lang=ALL-UNNAMED --add-exports java.base/sun.security.action=ALL-UNNAMED !JVM_ARGS!

set JVM_ARGS=-Djava.awt.headless=true !JVM_ARGS!
set TOOL_JAVA_CMD_QUOTED=!JAVA_CMD_QUOTED! !JVM_ARGS! -jar "!WLP_INSTALL_DIR!\bin\tools/ws-generateddlutil.jar"

@REM Execute the tool script or JAR.
if exist "!WLP_INSTALL_DIR!\lib\tools-internal/ddlGen.bat" goto:script
...
:script
set JAVA_RC=
call "!WLP_INSTALL_DIR!\lib\tools-internal/ddlGen" %*
if defined JAVA_RC (
  set RC=!JAVA_RC!
  call:javaCmdResult
)
